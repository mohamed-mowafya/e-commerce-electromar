...
const updateProfile = async (req, res, next) => {
  ...
  passport.authenticate("local", (err, user, info) => {
    if (err) return res.status(403).json({"status": "An authentication error has occured."})
    else {
      req.logIn(user, async (err) => {
        if (err) return res.status(403).json({ "status": "The provided old password is wrong." });
